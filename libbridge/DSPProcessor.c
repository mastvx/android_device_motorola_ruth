...
#ifdef DSPBRIDGE_DUMP_DBG
/*
 *  ======== DSPProcessor_dump_SHM========
 *  Purpose:
 *      will handle the user application for debuginfo
 */
DBAPI DSPProcessor_dump_SHM(void)
{
       DSP_STATUS status = DSP_SOK;
       Trapped_Args tempStruct;
       status = DSPTRAP_Trap(&tempStruct, CMD_PROC_DUMPSHM_OFFSET);
       return status;
}
#endif
